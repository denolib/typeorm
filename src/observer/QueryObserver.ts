...
export class QueryObserver {
    ...
    private insertEvents: InsertEvent<any>[] = [];
    private updateEvents: UpdateEvent<any>[] = [];
    private removeEvents: RemoveEvent<any>[] = [];
    ...
    observe(): Observable<any> {
        this.manager.observers.push(this);
        return new Observable(subscriptionObserver => {
            ...
            return () => {
                ...
                const index = this.manager.observers.indexOf(this);
                if (index !== -1)
                    this.manager.observers.splice(index, 1);
            };
        });
    }

    /**
     * Executes process of event dispatching to all subscribers based on collected events.
     */
    execute() {
        this.insertEvents.forEach(event => this.handleInsertEvent(event));
        this.updateEvents.forEach(event => this.handleUpdateEvent(event));
        this.removeEvents.forEach(event => this.handleRemoveEvent(event));
    ...
    private handleInsertEvent(event: InsertEvent<any>) {
    ...
    private handleUpdateEvent(event: UpdateEvent<any>) {
    ...
    private handleRemoveEvent(event: RemoveEvent<any>) {
