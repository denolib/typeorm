...
describe("uuid-cockroach", () => {
    ...
    test("should make correct schema with CockroachDB uuid type", () => Promise.all(connections.map(async connection => {
        const queryRunner = connection.createQueryRunner();
        const table = await queryRunner.getTable("record");
        await queryRunner.release();
        expect(table).toBeDefined();
        expect(table!.columns.find(tableColumn => tableColumn.name === "id" && tableColumn.type === "uuid" && tableColumn.isGenerated)).toBeDefined();
    })));
