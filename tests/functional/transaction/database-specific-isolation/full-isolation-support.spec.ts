...
describe("transaction > transaction with full isolation support", () => {
    ...
    test("should execute all operations in a single transaction with READ COMMITTED isolation level", () => Promise.all(connections.map(async connection => {
    ...
    test("should execute all operations in a single transaction with REPEATABLE READ isolation level", () => Promise.all(connections.map(async connection => {
