...
describe("query builder > relational query builder > add operation > one to many relation", () => {
    ...
    test("should handle addAndRemove method as well", () => Promise.all(connections.map(async connection => {
        ...
        expect(loadedCategory!.posts).toContainEqual({ id: 1, title: "post #1" });
        expect(loadedCategory!.posts).not.toContainEqual({ id: 2, title: "post #2" });
        expect(loadedCategory!.posts).toContainEqual({ id: 3, title: "post #3" });
    })));
