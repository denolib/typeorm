...
describe("query builder > relation-id > many-to-many > basic-functionality", () => {
    ...
    test("should not load ids when RelationId decorator is not specified", () => Promise.all(connections.map(async connection => {
        ...
        expect(loadedPosts![0].tagId).toBeUndefined();
        expect(loadedPosts![0].categories).not.toEqual([]);
        expect(loadedPosts![0].categoryIds).toBeUndefined();
        expect(loadedPosts![1].tag).not.toBeUndefined();
        expect(loadedPosts![1].tagId).toBeUndefined();
        expect(loadedPosts![1].categories).not.toEqual([]);
        expect(loadedPosts![1].categoryIds).toBeUndefined();
        ...
        expect(loadedPost!.tagId).toBeUndefined();
        expect(loadedPost!.categories).not.toEqual([]);
        expect(loadedPost!.categoryIds).toBeUndefined();

    })));

    test("should load ids when loadRelationIdAndMap used on ManyToMany owner side", () => Promise.all(connections.map(async connection => {
        ...
        expect(loadedPosts![0].categoryIds).not.toBeUndefined();
        expect(loadedPosts![0].categoryIds[0]).toEqual(1);
        expect(loadedPosts![0].categoryIds[1]).toEqual(2);
        expect(loadedPosts![1].categoryIds).not.toBeUndefined();
        expect(loadedPosts![1].categoryIds[0]).toEqual(1);
        expect(loadedPosts![1].categoryIds[1]).toEqual(2);
        ...
        expect(loadedPost!.categoryIds).not.toBeUndefined();
        expect(loadedPost!.categoryIds[0]).toEqual(1);
        expect(loadedPost!.categoryIds[1]).toEqual(2);

    })));

    test("should load ids when loadRelationIdAndMap used on ManyToMany owner side without inverse side", () => Promise.all(connections.map(async connection => {
    ...
    test("should load ids when loadRelationIdAndMap used on ManyToMany inverse side", () => Promise.all(connections.map(async connection => {
        ...
        expect(loadedCategory!.postIds).not.toBeUndefined();
        expect(loadedCategory!.postIds[0]).toEqual(1);
        expect(loadedCategory!.postIds[1]).toEqual(2);

    })));

    test("should load ids when loadRelationIdAndMap used on ManyToMany owning side with additional condition", () => Promise.all(connections.map(async connection => {
    ...
    test("should load ids when loadRelationIdAndMap used on ManyToMany owning side without inverse side and with additional condition", () => Promise.all(connections.map(async connection => {
    ...
    test("should load ids when loadRelationIdAndMap used on ManyToMany inverse side with additional condition", () => Promise.all(connections.map(async connection => {
        ...
        expect(loadedCategory!.postIds).not.toBeUndefined();
        expect(loadedCategory!.postIds.length).toEqual(1);
        expect(loadedCategory!.postIds[0]).toEqual(1);
    ...
    test("should load ids when loadRelationIdAndMap used on nested relation with additional conditions", () => Promise.all(connections.map(async connection => {
        ...
        expect(loadedPost!.categoryIds[0]).toEqual(1);
        expect(loadedPost!.categories[0].imageIds).not.toEqual([]);
        expect(loadedPost!.categories[0].imageIds.length).toEqual(1);
        expect(loadedPost!.categories[0].imageIds[0]).toEqual(1);

    })));

    test("should not load ids of nested relations when loadRelationIdAndMap used on inherit relation and parent relation was not found", () => Promise.all(connections.map(async connection => {
