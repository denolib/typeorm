...
describe("embedded > embedded-many-to-one-case5", () => {
    ...
    describe("owner side", () => {
        ...
        test("should insert, load, update and remove entities with embeddeds when embedded entity having ManyToOne relation with multiple primary keys (multiple keys in both sides)", () => Promise.all(connections.map(async connection => {
