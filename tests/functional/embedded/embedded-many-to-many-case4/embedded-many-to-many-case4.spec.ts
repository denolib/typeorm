...
describe("embedded > embedded-many-to-many-case4", () => {
    ...
    describe("owner side", () => {
        ...
        test("should insert, load, update and remove entities with embeddeds when embedded entity having ManyToMany relation with multiple primary keys (multiple keys in related entity)", () => Promise.all(connections.map(async connection => {
    ...
    describe("inverse side", () => {
        ...
        test("should insert, load, update and remove entities with embeddeds when embedded entity having ManyToMany relation with multiple primary keys (multiple keys in related entity)", () => Promise.all(connections.map(async connection => {
            ...
            expect(loadedUsers[1].name).toEqual("Clara");
        })));
