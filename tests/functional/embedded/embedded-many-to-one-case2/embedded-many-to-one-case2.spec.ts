...
describe("embedded > embedded-many-to-one-case2", () => {
    ...
    describe("owner side", () => {
        ...
        test("should insert, load, update and remove entities with embeddeds when embedded entity having ManyToOne relation", () => Promise.all(connections.map(async connection => {
    ...
    describe("inverse side", () => {
        ...
        test("should insert, load, update and remove entities with embeddeds when embedded entity having ManyToOne relation", () => Promise.all(connections.map(async connection => {
            ...
            expect(loadedUser!.likedPost).toBeNull();
        })));
    });
});
