...
describe("github issues > #175 ManyToMany relation doesn't put an empty array when the relation is empty", () => {
    ...
    test("should return post with categories if they are attached to the post", () => Promise.all(connections.map(async connection => {
    ...
    test("should return post with categories even if post with empty categories was saved", () => Promise.all(connections.map(async connection => {
    ...
    test("should return post with categories even if post was saved without categories set", () => Promise.all(connections.map(async connection => {
